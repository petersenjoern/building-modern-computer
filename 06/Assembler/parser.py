...
class Parser():
    '''Class Parser look at next token'''
    ...
    def advance(self):
        ...
        if tok == Lexicon.OP and val == '@':
            self._a_command()
        elif tok == Lexicon.OP and val == '(':
            self._l_command()
        else:
    ...
    def _get_comp(self, tok, val):
        if tok == Lexicon.OP and (val == '-' or val == '!'):
            tok2, val2 = self.lex.next_token()
            self._comp = val+val2
        elif tok == Lexicon.NUM or tok == Lexicon.ID:
            self._comp = val
            tok2, val2 = self.lex.peek_token()
            if tok2 == Lexicon.OP and val2 != ';':
                self.lex.next_token()
